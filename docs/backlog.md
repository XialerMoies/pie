...
- **桌面端无界前端边界收口**：主区域、功能栏、顶部栏和底部栏的边界按布局职责重新治理；移除面板内部多余分隔线，保留编辑器与设置内容区所需边界，并补齐相邻边界的圆角衔接。
- **问题栏交互完善**：移除标题下分隔线，增加顶部拖拽句柄，支持鼠标与键盘调整高度；问题栏展开或尺寸变化时 Token Rail 自动重新定位，避免遮挡输入区内容。
- **设置页边界调整**：标题栏与标签栏保持无边框，内容区保留顶部/左侧边界并在左上交界处使用圆角。
- **验证结果**：设置页测试 `83/83`、问题栏测试 `7/7`、布局与渲染测试 `95/95`，`npm run build` 通过。构建仍有既有 Monaco 大体积 chunk 警告，不影响产物生成。
...
- 2026-08-15 / 技术债 / **结构治理收官**：方法论与"已决策不再拆"清单固化进 CLAUDE.md（冻结→搬移→门禁→全量验证 + 依赖方向原则 + agent 三大文件/event-router/supervisor/permission-service authorizer 不拆理由），测试数字刷新（约 1,774）；profile 标记收工并恢复功能主线优先级 / 无产品代码变更；CLAUDE.md 本地生效（gitignored）
- 2026-08-15 / 技术债 / dashboard-settings 组件树拆分（曾是全代码库最大文件 947 行）：facade 947→214 行仅保留弹窗/事件委托/页签路由/兼容 API；通用设置（`settings-general.ts` 242）、模型厂商（`settings-provider-model.ts` 226）、自定义 Agent（`settings-custom-subagents.ts` 205）、存储设置（`settings-storage.ts` 176）各自为 XxxDependencies 构造注入 + 合并式注册（`...(existing), key`）的 owner 模块，owner 内 **App.\* 直接引用归零**（全前端 210→178、9→8 文件）；新增门禁：bundle 顺序（owners 必须在 facade 捕获 App.SettingsComponents 之前——facade 第 4 行即捕获引用）、职责回流（facade 无 controller 类/fetch/owner 状态变量）、400 行上限、XSS sink 基线迁移 13 个旧 sink + 新增 11 个新 sink 全部分类；dashboard.d.ts 新增 4 个 Api 接口 + AppSettingsComponents / typecheck、frontend 359/359、settings 20/20、多实例 12/12、Routes 339/339 + workspace-lock 13/13、smoke 84/84、bundle；提交 `9e7fdac`
- 2026-08-15 / 技术债 / workspace-lock queued-record flake 治理：移除 `setTimeout(100ms)` 固定睡眠竞态，改为 `waitFor` 服务器进入 graceful shutdown（`[server] stdin, shutting down` 标记）；新增中间态断言——锁占用期间服务器必须存活（exitCode===null）且 queued workspace 记录仍被阻塞（旧记录不变）；释放锁后先等锁持有者彻底退出再等 server 退出；fetch failed / 落盘失败断言附带子进程 stdout/stderr，子进程提前退出时可直接获得现场 / 压力 20/20 轮 40/40、完整 workspace-lock 13/13、Routes 339/339 + workspace-lock 13/13；提交 `d8116dc`
- 2026-08-15 / 技术债 / XSS 门禁扩展捕获 `+=`：SINK_ASSIGNMENTS 区分 `=`（innerHTML）与 `+=`（innerHTML+=），identity 携带操作符后缀；explorer `initTree` 内 3 处 `container.innerHTML +=` 静态 UI 文案作为 `initTree|innerHTML+=|container` 分组（count 3, REVIEW.static）录入基线；覆盖 **104 结构分组 / 138 sink** / 门禁 2/2、frontend 358/358、bundle；提交 `8389bf2`
...
- 2026-08-15 / 技术债 / 首批 XSS sink 审计门禁（未改产品运行时代码）：新增 `frontend-xss-sinks.test.mjs`——TypeScript AST 扫描前端 innerHTML/outerHTML 赋值 + insertAdjacentHTML 调用（103 结构分组 / 135 sink），以 file|函数|sink|目标 四元组 + count + RHS SHA-256 digest 锁定基线，任何新增 sink/修改 HTML 来源/函数归属变化触发 deepEqual 失败并要求复审；103 组全部标注信任分类（renderer 37 / escaped 24 / static 18 / clear 9 / markdown 8 / primitive 7）与理由；新增 Subagent/会话/MCP/权限 4 个真实 DOM 恶意载荷测试（走实际渲染路径 + E() 转义 + 断言无 img/script 注入、载荷保持可见文本）；门禁接入 test:frontend 首位 + test-serialization 防移除断言 / typecheck、frontend 358/358、unit 1056/1056、Routes 339/339 + workspace-lock 13/13、build smoke 80/80、bundle；SafeHtml/Trusted Types/CSP 留后续阶段；提交 `2b4e2a2`
- 2026-08-15 / 技术债 / 服务端 dashboard MCP 路由抽离：dashboard.ts 732→393 行仅保留统一入口与委派（MCP 委派仍在 waitForSessionReady 之后）；7 个 MCP API（servers/toggle/trust/catalog/install/install-custom/uninstall）+ 配置授权/信任/安装 helper 迁入 `dashboard-mcp.ts`（340 行），existingAncestorForPath/pathExists 迁入中性 `route-path-utils.ts`（20 行）；路由块与 helper 逐字一致；新增结构门禁（dashboard 无 /api/mcp、无 TrustStore/MCP_CATALOG、无反向依赖、路径工具中性化）/ typecheck、MCP 路由 13/13、MCP 权限 7/7、unit 1055/1055、Routes 339/339 + workspace-lock 13/13、frontend 352/352、build、bundle；提交 `b65d7f5`
- 2026-08-15 / 技术债 / 服务端 session-title 去重：usage-index（数据索引层）与 session-message-parser（解析层）各持一份 ~70 行完全一致的标题清洗/评分/截断（summarizeText/normalizeTitleLine/isGenericReplyIntro/scoreTitleLine/extractReplyTitle），迁入中性模块 `src/server/session-title.ts`（64 行，零依赖，不依赖 routes/usage-index）；两处各自保留 textFromBlocks 与薄层 deriveReplySummary，**非法 surrogate 清洗行为保持不变**（parser 清、usage-index 不清）；净删 ~50 行重复 / typecheck、定向 36/36、unit 1055/1055、Routes 338/338 + workspace-lock 13/13、frontend 352/352、bundle；提交 `ca3acc4`
- 2026-08-15 / 技术债 / 服务端 session-message-parser 抽离（sessions.ts 路由瘦身）：JSONL 解析（parseSessionMessages）、trace/block 回放（convertTracesToBlocks）、标题摘要（deriveReplySummary）迁入 `session-message-parser.ts`（395 行），sessions 939→547 行保留路由/文件定位/元数据读写 + 兼容 re-export；搬移主体与旧实现逐字节一致，仅新增 deriveReplySummary 导出；新增防回流门禁（parser 路由无关、sessions 不重定义解析），行为测试直载 parser / typecheck、sessions 24/24、unit 1053/1053、Routes 338/338 + workspace-lock 13/13、frontend 352/352、bundle；提交 `1c0f13c`
- 2026-08-15 / 技术债 / 服务端 permission-rule-manager 抽离（permission-service 拆分第二小批）：规则状态（workspace hydration / 规则 CRUD / 持久化更新 / scope 匹配 / 规则标准化）完整移入 `PermissionRuleManager` 类（371 行），service 968→744 行仅保留 facade、授权流程与 manager 委托；错误工厂保留 ServerPermissionError 状态码与错误码，公共类型（PermissionRuleListName/PermissionRulesSnapshot/ScopedPermissionRule）re-export 不变；新增防回流门禁（service 不得持有规则状态、manager 不反向依赖 service）/ typecheck、unit 1053/1053（Permission Service 84/84）、Routes、frontend 352/352、bundle；提交 `cfd11b4`
- 2026-08-15 / 技术债 / 服务端 permission-audit-log 抽离（permission-service 拆分第一小批）：审计核心（类型/过滤/序号/裁剪/持久化队列/reload/clear/flush）完整移入 `PermissionAuditLog` 类（119 行），service 仅委托 record/getTrail/flushWrites/clear（1028→968 行）；audit-store 改单向依赖 audit-log 消除类型循环，service/store 保留兼容导出；新增防回流门禁（service 不得持有审计状态、无循环依赖）/ typecheck、unit 1053/1053（Permission Service 83/83、共享存储锁 9/9）；提交 `fb2a11f`
- 2026-08-15 / 技术债 / Agent 层 dangerous-command.ts 拆分：facade 704→28 行纯编排；文本预检查（shell expansion + line break + CC 规则）与 legacy fallback（rm/git/chmod/del/removeItem + patternChecks）入 `text-rules.ts`、AST 命令族规则入 `ast-rules.ts`、类型/路径标准化/结果构造入 `shared.ts`；**安全判定顺序（文本→AST→legacy）与原实现一致**、公共 API 不变；新增规则优先级 + facade 防回流测试 / typecheck、command-security 390/390、unit 1053/1053、Routes 335+13、frontend 352/352；提交 `067e2e9`
- 2026-08-15 / 技术债 / Agent 层 path-extractors.ts 拆分：facade 714→89 行，类型/命令表/通用解析入 `core.ts`、复制/移动/find/sort/PowerShell 入 `filesystem.ts`、grep/rg/sed/jq 入 `search.ts`、Git/Tar 入 `archive.ts`；`extractCommandPathArgs` 单一公共入口与 50 命令映射不变；新增跨命令族行为快照（token/operation/source 冻结）+ 防回流门禁 / typecheck、command-security 388/388、unit 1051/1051、npm test 全量；提交 `2d94dd8`
- 2026-08-14 / 技术债 / App.* 依赖局部化第七批：dashboard-chat.ts 新增 `DashboardChatDependencies`（8 直接依赖 + 3 lazy getter——SessionTabs/SessionRestore/Git 晚注册模块惰性取），`DashboardChatApi` 扩展 App.Chat 额外方法，dashboardChat\* 专属别名避免 bundle 撞名；77 处 App.* → 0，装配层文件清到 0 证明"集中依赖门面"设计可行 / 全局剩余严格 App.* 9 文件 211 处（最大 dashboard-sessions 66）、typecheck、bundle、frontend 352/352；提交 `caa3cba`
