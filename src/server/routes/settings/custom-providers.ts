...
export const handleCustomProviderSettings: RouteHandler = async (req, res, ctx) => {
  ...
  try {
    ...
    if (route.kind === "list" && req.method === "GET") {
      writeJson(res, 200, await service.list(ctx.groups.providers.model.providerRuntime));
      return true;
    }
    ...
    if (route.kind === "list" && req.method === "POST") {
      ...
      const snapshot = await service.create(input, ctx.groups.providers.model.providerRuntime);
    ...
    const snapshot = req.method === "PUT"
      ? await service.update(providerId, input as CustomProviderMutationInput, ctx.groups.providers.model.providerRuntime)
      : await service.delete(providerId, input as CustomProviderDeleteInput, ctx.groups.providers.model.providerRuntime);
    const payload = JSON.stringify(snapshot);
    scheduleRuntimeSync(ctx);
