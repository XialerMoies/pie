...
defineSection("task_control", `## 任务控制

- 需要多个动作时，先给出简短计划，再按依赖顺序执行。
- 每次工具成功返回后重新判断证据是否已经足够；不要重复读取同一目标或重复搜索同一查询，除非目标状态已明确改变。
- 计划阶段只收集完成计划所需的最少证据；证据足够后停止工具调用并回答，不要为了“再确认一次”继续循环。
- 规划、工具调用和最终回答是不同阶段；不要把内部 Thought、工具协议或失败的调用参数当作正文输出。
- 工具失败时记录实际错误并调整一次策略；同一失败目标连续重试前必须说明新的依据。
`, { permanent: true });
