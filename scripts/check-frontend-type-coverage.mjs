...
function declarationFiles(dir) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    if (entry.name === "gen" || entry.name === "node_modules") continue;
    const full = resolve(dir, entry.name);
    if (entry.isDirectory()) result.push(...declarationFiles(full));
...
const declarations = declarationFiles(FRONTEND);
...
console.log(`[frontend-type-coverage] ${sources.length} source files + ${declarations.length} declarations covered; legacy global scripts syntax-only, strict modules use tsconfig.frontend.strict.json`);
